...
import { scheduleShootReminders } from '../services/NotificationService'
import { scheduleWhatsAppReminders } from '../services/WhatsAppService'
...
export default function AddOrder() {
  const { currentUser, canAddOrder, isTrialExpired, incrementOrderCount, studioData } = useAuth()
  ...
  // Paywall check (only for new orders, not edit)
  ...
  useEffect(() => {
    if (!isEdit) {
      if (isTrialExpired()) {
        setPaywallReason('trialExpired')
        setShowPaywall(true)
      } else if (!canAddOrder()) {
        setPaywallReason('orderLimit')
        setShowPaywall(true)
      }
    }
  }, [])
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isEdit) {
        ...
        const docRef = await addDoc(collection(db, 'studios', currentUser.uid, 'orders'), data)
        data.id = docRef.id
        
        // FCM Push Notifications
        await scheduleShootReminders(data, { ...studioData, uid: currentUser.uid })
        
        // WhatsApp Reminders (Pro plan check + user enabled)
        if ((studioData?.plan === 'pro' || studioData?.plan === 'studio') && studioData?.whatsappReminders) {
          await scheduleWhatsAppReminders(data, studioData)
        }
  ...
  return (
    <>
      {showPaywall && <Paywall reason={paywallReason} />}
